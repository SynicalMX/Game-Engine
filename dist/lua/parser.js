...
export default function parser(tokens) {
    ...
    function statement() {
        ...
        if (match('DO'))
            return { type: 'block', line: previous().line, body: block() };
        if (match('RETURN'))
            return { type: 'return', line: previous().line, value: expression() };
        return expression();
    }
    function ifStatement() {
        ...
        while (match('ELSEIF')) {
            ...
            elseIfs.set(elseIfCond, elseIfBody);
        }
        const elseBlock = [];
        if (match('ELSE')) {
    ...
    function assignment(doLowerPrecedence = true) {
        ...
        if (match('EQUALS')) {
            if (expr.type !== 'variable')
                error(expr.line, 'Expected variable name but got ' + expr.type);
            const value = expression();
    ...
    function primary() {
        ...
        if (match('LOCAL'))
            return { type: 'local', line: previous().line };
