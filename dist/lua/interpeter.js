...
export default function interpreter(trees, scope) {
    if (!scope) {
        ...
        scope.defineFunction('print', new LuaFunction((...objs) => {
            console.log("LUA: " + objs.map(obj => stringify(obj)).join(' '));
    ...
    function scopeSet(name, value) {
        if (scope.localVariables.hasOwnProperty.call(name))
            return scope.localVariables[name] = value;
        if (scope.globalVariables.hasOwnProperty.call(name))
            return scope.globalVariables[name] = value;
    }
    function type(obj) {
    ...
    function interpret(expr) {
        ...
        switch (expr.type) {
                ...
                switch (expr.operator) {
                    ...
                    case 'GREATER':
                        ...
                        if (typeof left !== typeof right)
                            error(expr.line, 'Cannot compare ' + type(left) + ' with ' + type(right));
                        return left > right;
                    case 'GREATER_EQUALS':
                        checkType(left, expr.left.line, 'Can only compare numbers or strings', 'number', 'string');
                        checkType(right, expr.right.line, 'Can only compare numbers or strings', 'number', 'string');
            ...
            case 'assignment': {
                ...
                if (scope.localVariables.hasOwnProperty.call(name) || scope.globalVariables.hasOwnProperty.call(name))
                    return scopeSet(name, interpret(expr.value));
                else {
                    if (expr.local)
                        return scope.localVariables[name] = interpret(expr.value);
                    else
                        return scope.globalVariables[name] = interpret(expr.value);
                }
            }
            case 'return':
            ...
            case 'block':
                ...
                return null;
            case 'if':
                if (truthy(interpret(expr.condition)))
                    interpretBlock(expr.body);
                else {
                    let doElse = true;
                    for (const [condition, block] of expr.elseIfs) {
                        const doBlock = truthy(interpret(condition));
