...
export default function interpreter(trees, scope) {
	...
	function scopeSet(name, value) {
		if (scope.localVariables.hasOwnProperty.call(name)) return scope.localVariables[name] = value;
		if (scope.globalVariables.hasOwnProperty.call(name)) return scope.globalVariables[name] = value;
	}
	function type(obj) {
